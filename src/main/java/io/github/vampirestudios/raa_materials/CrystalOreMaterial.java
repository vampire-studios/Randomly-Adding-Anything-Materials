...
public class CrystalOreMaterial extends OreMaterial {
	private static BufferTexture[] oreVeins;
	private static BufferTexture[] crystals;
	...
	public CrystalOreMaterial(Target targetIn, Random random) {
		super(targetIn, random);
		String regName = this.name.toLowerCase();
		crystal = InnerRegistry.registerItem(regName + "_crystal", new Item(new Settings().group(RAAMaterials.RAA_RESOURCES)));
		drop = crystal;
		...
		Artifice.registerDataPack(id(regName + "_ore_recipes"), dataPackBuilder -> {
			dataPackBuilder.addShapedRecipe(id(regName + "_helmet"), shapedRecipeBuilder -> {
				...
				shapedRecipeBuilder.ingredientItem('i', id(regName + "_crystal"));
			...
			dataPackBuilder.addShapedRecipe(id(regName + "_block_from_crystal"), shapedRecipeBuilder -> {
	...
	protected void initClientCustom(Random random) {
		...
		makeColoredItemAssets(crystals, crystal, gradient, random, regName + "_crystal", "%s Crystal");
	...
	private void loadStaticImages() {
		if (oreVeins == null) {
			oreVeins = new BufferTexture[9];
			for (int i = 0; i < oreVeins.length; i++) {
				oreVeins[i] = TextureHelper.loadTexture("textures/block/ores/crystals/ore_" + (i+1) + ".png");
				TextureHelper.normalize(oreVeins[i], 0.35F, 1F);
			}
			storageBlocks = new BufferTexture[3];
			for (int i = 0; i < storageBlocks.length; i++) {
				storageBlocks[i] = TextureHelper.loadTexture("textures/block/storage_blocks/crystals/crystal_" + (i+1) + ".png");
				TextureHelper.normalize(storageBlocks[i], 0.35F, 1F);
			}

			crystals = new BufferTexture[13];
			for (int i = 0; i < crystals.length; i++) {
				crystals[i] = TextureHelper.loadTexture("textures/item/crystals/crystal_" + (i+1) + ".png");
				TextureHelper.normalize(crystals[i]);
			}
